...
import dashboard.actions as actions


class FakeActionsCursor:
...
class FakeActionsConn:
    def __init__(self, row=None, repo_active=True):
        self.row = row
    ...
    def execute(self, sql, params=None):
        ...
        return FakeActionsCursor(normalized, params, self)
    ...
    def statuses_set(self):
        return [
            p[0] for sql, p in self.queries
            if sql.startswith("UPDATE pending_actions SET status")
            for p in [p]
        ]
...
def test_approve_action_expires_when_past_the_configured_ttl():
    ...
    conn = FakeActionsConn(row=old_row)
...
def test_approve_action_blocks_when_repo_is_not_active():
    row = _pending_row()
    conn = FakeActionsConn(row=row, repo_active=False)
    write_client = MagicMock()
